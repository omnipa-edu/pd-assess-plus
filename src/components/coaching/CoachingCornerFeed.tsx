...
import { useDismissCoaching } from '@/hooks/useCoachingCorner';
...
export function CoachingCornerFeed({ role, limit = 3, className }: CoachingCornerFeedProps) {
  ...
  useEffect(() => {
    if (items && items.length > 0) {
      const hasInstagram = items.some(
        item => item.content_type === 'instagram'
      );
      if (hasInstagram) {
  ...
  // Show the first (pinned or highest priority) item
  const primaryItem = visibleItems[0];
  
  // Map to CoachingItem format
  const coachingItem: CoachingItem = {
    id: primaryItem.id,
